...
describe('owns', () => {
  ...
  describe('authentication failures', () => {
    it('should throw ContractError when user is not authenticated', async () => {
      ...
      await expect(condition(input, noUserContext)).rejects.toThrow(ContractError);
      await expect(condition(input, noUserContext)).rejects.toThrow('User not authenticated for ownership check');
      ...
      try {
        ...
        expect((error as ContractError).category).toBe(ErrorCategory.AUTHENTICATION);
      }
    ...
    it('should throw ContractError when user ID is missing', async () => {
      ...
      await expect(condition(input, contextWithoutUserId)).rejects.toThrow('User not authenticated for ownership check');
    });
  });
  ...
  describe('validation failures', () => {
    it('should throw ContractError when resource ID field is missing', async () => {
      ...
      await expect(condition(input, ownerContext)).rejects.toThrow(ContractError);
      await expect(condition(input, ownerContext)).rejects.toThrow("Resource ID field 'resourceId' is missing in input");
    ...
    it('should throw ContractError when resource ID is null or undefined', async () => {
      ...
      await expect(condition(nullInput, ownerContext)).rejects.toThrow("Resource ID field 'resourceId' is missing in input");
      await expect(condition(undefinedInput, ownerContext)).rejects.toThrow("Resource ID field 'resourceId' is missing in input");
    });
  ...
  describe('resource not found failures', () => {
    it('should throw ContractError when resource is not found', async () => {
      ...
      await expect(condition(input, ownerContext)).rejects.toThrow(ContractError);
      await expect(condition(input, ownerContext)).rejects.toThrow('Resource with ID nonexistent123 not found');
    ...
    it('should throw ContractError when resource is undefined', async () => {
      ...
      await expect(condition(input, ownerContext)).rejects.toThrow('Resource with ID undefined123 not found');
    });
  });
  ...
  describe('ownership denial failures', () => {
    it('should throw ContractError when user does not own the resource', async () => {
      ...
      await expect(condition(input, ownerContext)).rejects.toThrow(ContractError);
      await expect(condition(input, ownerContext)).rejects.toThrow('User user123 does not own resource resource123');
      ...
      try {
        ...
        expect((error as ContractError).category).toBe(ErrorCategory.AUTHORIZATION);
      }
  ...
  describe('roles undefined handling', () => {
    ...
    it('should not grant admin privileges to users with undefined roles', async () => {
      ...
      await expect(condition(input, noRolesContext)).rejects.toThrow('User noroles123 does not own resource resource123');
    });
  ...
  describe('getResource error handling', () => {
    it('should propagate errors from getResource function', async () => {
      ...
      await expect(condition(input, ownerContext)).rejects.toThrow(resourceError);
      expect(mockGetResource).toHaveBeenCalledWith('resource123');
    });

    it('should handle getResource throwing ContractError', async () => {
      const contractError = new ContractError('Resource provider not configured', {
  ...
  describe('input parameter variations', () => {
    ...
    it('should handle special characters in resource field names', async () => {
      const specialFieldNames = ['resource-id', 'resource_id', 'resourceId$', 'resource.id'];
  ...
  describe('integration scenarios', () => {
    ...
    it('should deny access in multi-tenant scenario', async () => {
      ...
      await expect(fileCondition(deleteInput, ownerContext)).rejects.toThrow('User user123 does not own resource file789');
    });
