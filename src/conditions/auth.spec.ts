...
describe('auth', () => {
  ...
  describe('authentication failures', () => {
    it('should throw ContractError when user is not logged in', async () => {
      ...
      await expect(condition({}, noUserContext)).rejects.toThrow('User must be logged in');
    ...
    it('should throw ContractError when session is missing', async () => {
      ...
      await expect(condition({}, noSessionContext)).rejects.toThrow(ContractError);
      await expect(condition({}, noSessionContext)).rejects.toThrow('Session has expired');
    ...
    it('should throw ContractError when session is expired', async () => {
      ...
      await expect(condition({}, expiredContext)).rejects.toThrow(ContractError);
      await expect(condition({}, expiredContext)).rejects.toThrow('Session has expired');
  ...
  describe('authorization failures', () => {
    it('should throw ContractError when user lacks required role', async () => {
      ...
      await expect(condition({}, mockContext)).rejects.toThrow('Required role: admin, User roles: user, moderator');
    ...
    it('should throw ContractError with detailed role information', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(ContractError);
        expect((error as ContractError).message).toBe('Required role: admin, User roles: user');
      }
    });
    ...
    it('should handle empty roles array', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(ContractError);
        expect((error as ContractError).message).toBe('Required role: user, User roles: ');
      }
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle session expiry at exact current time', async () => {
      ...
      await expect(condition({}, nowExpiryContext)).rejects.toThrow('Session has expired');
    });
    ...
    it('should handle malformed session expiry date', async () => {
      ...
      await expect(condition({}, malformedExpiryContext)).rejects.toThrow('Session has expired');
    });
    ...
    it('should handle case-sensitive role matching', async () => {
      ...
      await expect(condition({}, mockContext)).rejects.toThrow(ContractError);
      await expect(condition({}, mockContext)).rejects.toThrow('Required role: Admin');
    });
  ...
  describe('integration scenarios', () => {
    it('should work in typical admin access scenario', async () => {
      ...
      const result = await adminCondition({ action: 'delete', resourceId: 'critical-data' }, adminContext);
    ...
    it('should fail in privilege escalation attempt', async () => {
      ...
      await expect(adminCondition({ action: 'admin-only' }, userOnlyContext)).rejects.toThrow(ContractError);
      await expect(adminCondition({ action: 'admin-only' }, userOnlyContext)).rejects.toThrow('Required role: admin');
    });
  });
});