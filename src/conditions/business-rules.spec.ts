...
describe("businessRule", () => {
      ...
      const complexInput = {
        order: {
          ...
          items: [
            ...
            { id: "item2", price: 25.5, quantity: 1 },
          ],
          total: 47.48,
          currency: "USD",
        },
  ...
  describe("business rule violations", () => {
    it("should throw ContractError when synchronous rule fails", async () => {
      ...
      await expect(condition(input, mockContext)).rejects.toThrow(
        ContractError
      );
  ...
  describe("input and context handling", () => {
    ...
    it("should pass correct context to rule function", async () => {
      ...
      const differentContexts = [
        ...
        {
          ...mockContext,
          user: { ...mockContext.user!, id: "different-user" },
        },
  ...
  describe("real-world business rule examples", () => {
    it("should validate positive order values", async () => {
      ...
      // Invalid cases
      await expect(condition({ value: 0 }, mockContext)).rejects.toThrow(
        "Order value must be positive"
      );
      await expect(condition({ value: -10 }, mockContext)).rejects.toThrow(
        "Order value must be positive"
      );
    });
    ...
    it("should validate user age requirements", async () => {
      ...
      // Invalid cases
      await expect(condition({ age: 17 }, mockContext)).rejects.toThrow(
        "User must be at least 18 years old"
      );
      await expect(condition({ age: 0 }, mockContext)).rejects.toThrow(
        "User must be at least 18 years old"
      );
    });
    ...
    it("should validate inventory levels", async () => {
      ...
      const inventoryRule = async (input: {
        productId: string;
        quantity: number;
      }) => {
      ...
      await expect(
        condition({ productId: "prod123", quantity: 50 }, mockContext)
      ).resolves.toBe(true);
    ...
    it("should validate user permissions with context", async () => {
      ...
      await expect(condition({ action: "read" }, mockContext)).resolves.toBe(
        true
      );

      // Regular user trying to delete
      await expect(
        condition({ action: "delete" }, mockContext)
      ...
      await expect(condition({ action: "delete" }, adminContext)).resolves.toBe(
        true
      );
    });

    it("should validate complex business constraints", async () => {
      const complexRule = async (
        input: {
          orderId: string;
      ...
      await expect(condition(invalidOrder, mockContext)).rejects.toThrow(
        "Order total must be at least $10"
      );
  ...
  describe("edge cases and corner scenarios", () => {
    it("should handle rule functions that return non-boolean values", async () => {
      ...
      await expect(falsyCondition({}, mockContext)).rejects.toThrow(
        "Falsy rule"
      );
    });
    ...
    it("should handle rule functions that return Promise of non-boolean values", async () => {
      ...
      const truthyCondition = businessRule(
        "Truthy async rule",
        truthyAsyncRule
      );
    ...
    it("should handle undefined and null return values", async () => {
      ...
      await expect(undefinedCondition({}, mockContext)).rejects.toThrow(
        "Undefined rule"
      );
