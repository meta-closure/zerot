...
describe('Contract Types and Functions', () => {
  ...
  describe('isValidator', () => {
    it('should return true for a valid validator function', () => {
      const validator: ContractValidator<string, number> = (input: string) => parseInt(input);
    ...
    it('should return false for a regular condition function', () => {
      const condition: ContractCondition<string, AuthContext> = (input, context) => {
  ...
  describe('Session Provider', () => {
    it('should set and use a session provider', async () => {
      ...
      const mockProvider: SessionProvider = vi.fn().mockResolvedValue(mockAuthContext);
    ...
    it('should handle synchronous session provider', async () => {
      ...
      const mockProvider: SessionProvider = vi.fn().mockReturnValue(mockAuthContext);
      setSessionProvider(mockProvider);
    ...
    it('should handle session provider throwing an error', async () => {
      const mockProvider: SessionProvider = vi.fn().mockRejectedValue(new Error('Provider error'));
      setSessionProvider(mockProvider);
  ...
  describe('Resource Provider', () => {
    it('should set and use a resource provider', async () => {
      ...
      const mockProvider: ResourceProvider = vi.fn().mockResolvedValue(mockResource);
    ...
    it('should handle resource provider throwing an error', async () => {
      const mockProvider: ResourceProvider = vi.fn().mockRejectedValue(new Error('Database error'));
      setResourceProvider(mockProvider);

      await expect(getResource('resource123')).rejects.toThrow('Database error');
    });
  });
  ...
  describe('Type Definitions', () => {
    describe('ContractCondition', () => {
      ...
      it('should accept function returning Promise<boolean>', () => {
        const condition: ContractCondition<string, AuthContext> = async (input, context) => {
      ...
      it('should accept function returning ContractError', () => {
        const condition: ContractCondition<string, AuthContext> = (input, context) => {
          return new ContractError('Validation failed', { category: ErrorCategory.VALIDATION });
        };
      ...
      it('should accept function returning Promise<ContractError>', () => {
        const condition: ContractCondition<string, AuthContext> = async (input, context) => {
          return Promise.resolve(new ContractError('Async validation failed', { category: ErrorCategory.VALIDATION }));
        };
    ...
    describe('ContractValidator', () => {
      ...
      it('should work without explicit isValidator property', () => {
        const validator: ContractValidator<string, string> = (input: string) => input.trim();
    ...
    describe('ContractEnsuresCondition', () => {
      it('should accept function with output, input, and context parameters', () => {
        const ensuresCondition: ContractEnsuresCondition<number, string, AuthContext> = 
    ...
    describe('ContractInvariant', () => {
      it('should accept function with input and output parameters', () => {
        const invariant: ContractInvariant<string, number> = (input, output) => {
      ...
      it('should accept async function', () => {
        const invariant: ContractInvariant<string, number> = async (input, output) => {
      ...
      it('should accept all optional properties', () => {
        const options: ContractOptions<string, number, AuthContext> = {
          requires: [
            ...
            ((input: string) => input.trim()) as ContractValidator<string, string>
          ],
          ensures: [
            (output, input, context) => output > 0
          ],
          invariants: [
            (input, output) => input.length >= 0
          ],
          layer: 'business',
          retryAttempts: 3,
          retryDelayMs: 500,
  ...
  describe('Integration Tests', () => {
    it('should work with complex workflow', async () => {
      ...
      setSessionProvider(() => Promise.resolve({
        user: mockUser,
        session: mockSession
      }));
    ...
    it('should handle validator identification correctly', () => {
      // Create a validator
      const numberValidator: ContractValidator<string, number> = (input: string) => {
        const num = parseInt(input);
        if (isNaN(num)) {
          throw new ContractError('Invalid number', { category: ErrorCategory.VALIDATION });
        }
        return num;
      };
      ...
      const lengthCondition: ContractCondition<string, AuthContext> = (input, context) => {
