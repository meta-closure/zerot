...
class TestService {
  ...
  @contract({
    ...
    invariants: [
      ...
      async (input: TestInput, output: TestOutput) =>
  ...
  async methodWithFailingPrecondition(
    input: TestInput,
    context?: TestAuthContext
  ): Promise<TestOutput> {
    return {
  ...
  async methodWithFailingPostcondition(
    input: TestInput,
    context?: TestAuthContext
  ): Promise<TestOutput> {
    return {
  ...
  @contract({
    invariants: [async (input: TestInput, output: TestOutput) => false],
    layer: "business" as const,
  })
  async methodWithFailingInvariant(
    input: TestInput,
    context?: TestAuthContext
  ): Promise<TestOutput> {
    return {
  ...
  async methodWithRetryableError(
    input: TestInput,
    context?: TestAuthContext
  ): Promise<TestOutput> {
    return {
  ...
  async methodWithCategoryBasedRetry(
    input: TestInput,
    context?: TestAuthContext
  ): Promise<TestOutput> {
    return {
...
describe("contract decorator", () => {
  ...
  describe("preconditions (requires)", () => {
    ...
    it("should throw ContractViolationError when precondition returns false", async () => {
      ...
      await expect(service.methodWithConditions(input)).rejects.toThrow(
        ContractViolationError
      );
    });
    ...
    it("should throw ContractViolationError when precondition returns ContractError", async () => {
      ...
      await expect(
        service.methodWithFailingPrecondition(input)
    ...
    it("should wrap unexpected errors in ContractViolationError", async () => {
      ...
      await expect(
        errorService.methodWithUnexpectedError(input)
      ).rejects.toThrow(ContractViolationError);
    });
  });
  ...
  describe("postconditions (ensures)", () => {
    it("should throw ContractViolationError when postcondition returns ContractError", async () => {
      ...
      await expect(
        service.methodWithFailingPostcondition(input)
      ).rejects.toThrow(ContractViolationError);
    ...
    it("should throw ContractViolationError when postcondition returns false", async () => {
      ...
      class TestServiceWithFalsePostcondition extends TestService {
        ...
        async methodWithFalsePostcondition(
          input: TestInput
        ): Promise<TestOutput> {
          return {
            id: "1",
      ...
      await expect(
        errorService.methodWithFalsePostcondition(input)
      ).rejects.toThrow(ContractViolationError);
    });
  });
  ...
  describe("invariants", () => {
    it("should throw ContractViolationError when invariant returns false", async () => {
      ...
      await expect(service.methodWithFailingInvariant(input)).rejects.toThrow(
        ContractViolationError
      );
    ...
    it("should throw ContractViolationError when invariant returns ContractError", async () => {
      ...
      class TestServiceWithErrorInvariant extends TestService {
        @contract({
          invariants: [
            async () =>
              new ContractError("Invariant error", {
      ...
      await expect(
        errorService.methodWithErrorInvariant(input)
      ).rejects.toThrow(ContractViolationError);
    });
  });
  ...
  describe("retry functionality", () => {
    it("should retry on recoverable errors", async () => {
      ...
      await expect(service.methodWithRetryableError(input)).rejects.toThrow(
        ContractViolationError
      );
    ...
    it("should retry on specific error categories", async () => {
      ...
      await expect(service.methodWithCategoryBasedRetry(input)).rejects.toThrow(
        ContractViolationError
      );
