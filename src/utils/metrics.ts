...
export class Metrics {
  ...
  static increment(name: string, labels?: Record<string, string>): void {
    ...
    const currentValue = metric.data.length > 0 ? metric.data[metric.data.length - 1].value : 0;
    metric.data.push({ value: currentValue + 1, timestamp: performance.now(), labels });
  }
   ...
   * @param labels - Optional labels to associate with this metric.
   */
  static record(name: string, value: number, labels?: Record<string, string>): void {
   ...
   * @private
   */
  private static ensureMetric(name: string, type: 'counter' | 'gauge' | 'histogram'): void {
    if (!this.metrics.has(name)) {
      this.metrics.set(name, { type, data: [] });
    } else if (this.metrics.get(name)!.type !== type) {
      console.warn(`Metric '${name}' already exists with type '${this.metrics.get(name)!.type}'. Cannot re-register with type '${type}'.`);
    }
  }
  ...
  static getReport(): string {
    ...
    this.metrics.forEach((metric, name) => {
      if (metric.type === 'counter') {
        report[name] = {
          type: 'counter',
          total: metric.data.length > 0 ? metric.data[metric.data.length - 1].value : 0,
          entries: metric.data.map(d => ({ value: d.value, timestamp: d.timestamp, labels: d.labels }))
        };
      } else if (metric.type === 'gauge') {
        report[name] = {
          type: 'gauge',
          current: metric.data.length > 0 ? metric.data[metric.data.length - 1].value : undefined,
          entries: metric.data.map(d => ({ value: d.value, timestamp: d.timestamp, labels: d.labels }))
        };
        ...
        report[name] = {
          ...
          min: count > 0 ? parseFloat(Math.min(...values).toFixed(2)) : undefined,
          max: count > 0 ? parseFloat(Math.max(...values).toFixed(2)) : undefined,
          entries: metric.data.map(d => ({ value: d.value, timestamp: d.timestamp, labels: d.labels }))
        };
      }
    });
